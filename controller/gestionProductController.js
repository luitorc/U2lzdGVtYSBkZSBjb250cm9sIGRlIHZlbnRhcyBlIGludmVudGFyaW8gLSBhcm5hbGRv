		...
		producto_model.getData({},function(obj1){
			for( ii in listProduct ){
				for( i in obj1 ){
					if( listProduct[ii].id_producto == obj1[i].id_producto ){
						...
						obj1[i].subTotalProveedor = obj1[i].precio_proveedor * parseInt(listProduct[ii].cantList);
						total_proveedor = total_proveedor + parseFloat(obj1[i].subTotalProveedor);
						//saco de cokkie
			...
			if(tool == undefined){
				...
				req.session.venta = {
					...
					, total_proveedor: total_proveedor
					, discount: {
						state: tool.discount_state
						,percent: tool.discount_percent
						,price: tool.discount_price
					}
				};
				res.send({
					listProductR: listProductR
					, total:total
					, total_proveedor: total_proveedor
					,discount_state: tool.discount_state
					,discount_percent: tool.discount_percent
					,discount_price: tool.discount_price
				});
